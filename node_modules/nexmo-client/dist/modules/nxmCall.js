...
class NXMCall {
    ...
    */
    _setupConversationObject(conversation, rtc_id) {
        ...
        // Attempt to enable rtc call stats reporting for prewarm leg calls
        if (rtc_id && this.rtcObjects && this.rtcObjects[rtc_id] && this.rtcObjects[rtc_id].pc) {
            rtc_helper_1.default.enableCallStats({ application: this.application, conversation, pc: this.rtcObjects[rtc_id].pc });
        }
   ...
   * @param {string} type the type of the call you want to have. possible values "phone" or "app" (default is "phone")
    ...
    */
    async createServerCall(user, type) {
        ...
        try {
            // enable stats events emit if configured and store on call Object
            if (((this.application.session.config || {}).rtcstats || {}).emit_events) {
                const rtcStats = rtc_helper_1.default._initStatsEvents(this.application, this);
                this.rtcStats = rtcStats;
            }
            // PrewarmLeg
            const { stream, legId, rtcObjects } = await rtc_helper_1.default.prewarmLeg(this.application);
